...
flights |> 
  ...
  arrange(arr_delay)
```
The code doesn't work because the select drops the other columns, including arr_delay; the error means that the non-existant column cannot be found in the new, smaller data frame.
